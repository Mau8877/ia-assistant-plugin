(function () {
  ...
  window.IA_Components.Revision.generarResumen = function ($el) {
    ...
    $el.find(".ia-comp-quiz .ia-pregunta").each(function () {
      ...
      var respuestaTexto =
        $seleccionada.length > 0
          ? escaparHTML($seleccionada.parent().text().trim())
          : "Sin responder";
    ...
    $el.find(".ia-comp-abierta").each(function () {
      ...
      var respuestaTexto =
        textoAlumno.length > 0 ? escaparHTML(textoAlumno) : "Sin responder";
      var clases = textoAlumno.length > 0 ? "" : "es-vacio";
      htmlFinal += construirTarjeta(
    ...
    if (htmlFinal === "")
      htmlFinal =
        '<div class="empty-state">No hay componentes evaluables.</div>';
