function StudentMasterInit(runtime, element) {
  ...
  function obtenerDataRespuestas() {
    ...
    $el.find(".ia-comp-abierta").each(function () {
      ...
      if (idVal)
        dataFinal.respuestas_abiertas.push({
          id: idVal,
    ...
    $el.find(".ia-comp-codigo").each(function () {
      var idVal = $(this).attr("id");
      if (idVal)
        dataFinal.respuestas_codigo.push({
          id: idVal,
  ...
  function restaurarMemoria() {
    try {
      ...
      var intentosAgotados =
        $el.find("#ia-intentos-data").text().trim() === "true";
  ...
  function organizarComponentes() {
    ...
    $el.find(".tab-content").each(function () {
      if (
        $(this)
          .children()
          .not(
            ".empty-state, h3, #revision-summary, .ia-master-actions, p, hr, #master-feedback, .ia-revision-container",
          ).length > 0
      ) {
    ...
    if ($primeraPestana.length > 0) {
      ...
      $el
        .find("#" + $primeraPestana.data("target"))
        .show()
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.resultado === "ok") {
          ...
          $el
            .find(
              ".ia-pregunta, .ia-comp-abierta, .ia-comp-codigo, .ia-comp-quiz",
            )
          ...
          data.feedback.forEach(function (item) {
            ...
            if ($target.length > 0) {
              ...
              if ($fbContainer.length > 0) {
                $fbContainer
                  .find(".fb-text")
                  .html("<b>Feedback:</b> " + item.detalle);
                $fbContainer.removeAttr("style").css({
                  display: "block",
                  visibility: "visible",
                  opacity: "1",
                });
              ...
              if (item.tipo === "Quiz" || $target.hasClass("ia-comp-quiz")) {
                $target.find(".ia-pregunta").each(function () {
                  ...
                  if (
                    ...
                    String(seleccionada) === correcta
                  )
                    $q.addClass("is-correct");
          ...
          $feedbackArea
            .html('<p class="error-msg">❌ ' + data.mensaje + "</p>")
            .show();
        }
      },
      error: function (xhr, status, error) {
        $btn.text("Enviar Evaluación Completa").prop("disabled", false);
        $feedbackArea
          .html('<p class="error-msg">❌ Error de conexión: ' + error + "</p>")
          .show();
      },
    });
  });
