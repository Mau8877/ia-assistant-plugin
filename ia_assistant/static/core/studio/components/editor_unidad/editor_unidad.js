...
window.IA_Components.EDITOR_UNIDAD = class {
  ...
  DISTRIBUIR_COMPONENTES_VISUALES(data) {
    ...
    // Validación de seguridad para Quill
    if (typeof window.Quill === "undefined") {
      console.error(
        "[IA Assistant] Quill no está disponible en window. Verifica carga de vendors.",
      );
      this.MOSTRAR_ERROR(
        "Error cargando el editor de texto (Quill). Recarga la página.",
      );
      if (typeof window.IA_RENDER_LUCIDE_ICONS === "function") {
        window.IA_RENDER_LUCIDE_ICONS(el);
      }
      return;
    }

    // Inicializar Quill después de inyectar el HTML dinámico
    $(".quill-container", el).each(function () {
      if ($(this).data("quill-instance")) return;
    ...
    // Renderizar iconos de Lucide para contenido dinámico recién inyectado
    if (typeof window.IA_RENDER_LUCIDE_ICONS === "function") {
      window.IA_RENDER_LUCIDE_ICONS(el);
    } else if (window.lucide && typeof window.lucide.createIcons === "function") {
      window.lucide.createIcons({ root: el });
    } else {
      console.warn("[IA Assistant] Lucide no está disponible globalmente.");
    }
  }
